...
for (const file of dockerfiles) {
  ...
  assert.ok(content.includes('ARG HIVE_MIND_VERSION=latest'), `${file} should accept the exact published auto-programmer version as a build arg`);
  ...
  assert.ok(content.includes('bun install -g "auto-programmer@${HIVE_MIND_VERSION}"'), `${file} should install the published auto-programmer package version`);
  assert.ok(content.includes('test "$(hive --version)" = "${HIVE_MIND_VERSION}"'), `${file} should verify the installed auto-programmer version when a release version is supplied`);
...
// (2) actually build the Docker image using auto-programmer@latest
...
assert.ok(releaseYml.includes('name: auto-programmer-digests-${{ matrix.platform =='), 'release Docker artifacts should use the normal image digest namespace');
assert.ok(releaseYml.includes('pattern: auto-programmer-digests-*'), 'release Docker merge should download only normal image digests');
assert.ok(releaseYml.includes('name: auto-programmer-instant-digests-${{ matrix.platform =='), 'instant Docker artifacts should use the normal image digest namespace');
assert.ok(releaseYml.includes('pattern: auto-programmer-instant-digests-*'), 'instant Docker merge should download only normal image digests');
assert.ok(releaseYml.includes('name: auto-programmer-dind-digests-${{ matrix.platform =='), 'release DinD artifacts should use a separate digest namespace');
assert.ok(releaseYml.includes('pattern: auto-programmer-dind-digests-*'), 'release DinD merge should download only DinD digests');
assert.ok(releaseYml.includes('name: auto-programmer-dind-instant-digests-${{ matrix.platform =='), 'instant DinD artifacts should use a separate digest namespace');
assert.ok(releaseYml.includes('pattern: auto-programmer-dind-instant-digests-*'), 'instant DinD merge should download only DinD digests');
