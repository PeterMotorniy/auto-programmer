 ...
 *
 * Regression coverage for issue #1705: Auto Programmer must publish a separate
 * Docker-in-Docker image so agents can run Docker-based project tests inside
 * the Auto Programmer container without replacing the existing image.
 */
...
assertIncludes(dindDockerfile, 'FROM petermotorniy/box-dind:2.3.5', 'Dockerfile.dind');
...
assertIncludes(dindDockerfile, 'bun install -g "auto-programmer@${HIVE_MIND_VERSION}"', 'Dockerfile.dind');
...
assertIncludes(dockerDocs, 'petermotorniy/auto-programmer-dind:latest', 'docs/DOCKER.md');
...
assertIncludes(dockerPrSection, 'DIND_IMAGE_NAME: petermotorniy/auto-programmer-dind', '.github/workflows/release.yml');
...
assertIncludes(verifyDindExecDefaults, 'container_name="auto-programmer-dind-verify"', 'scripts/verify-dind-exec-defaults.sh');
...
assertIncludes(releaseYml, 'IMAGE_NAME: petermotorniy/auto-programmer-dind', '.github/workflows/release.yml');
assertIncludes(releaseYml, 'file: ./Dockerfile.dind', '.github/workflows/release.yml');
assertIncludes(releaseYml, 'auto-programmer-dind-digests-${{ matrix.platform ==', '.github/workflows/release.yml');
assertIncludes(releaseYml, 'auto-programmer-dind-instant-digests-${{ matrix.platform ==', '.github/workflows/release.yml');
